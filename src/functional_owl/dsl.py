...
if importlib.util.find_spec("bioregistry"):

    def _upgrade_ref(
        reference: bioregistry.NormalizedReference | curies.Reference,
    ) -> curies.Reference:
...
class IdentifierBox(Box):
    ...
    def __init__(self, identifier: IdentifierBoxOrHint) -> None:
        """Initialize the identifier box with a URIRef, Reference, or string CURIE."""
...
def get_owl_restriction(
    ...
    restriction_target: Box | term.Node,
    converter: Converter,
) -> term.BNode:
...
class ObjectPropertyChain(Box):
    ...
    def to_rdflib_node(self, graph: Graph, converter: Converter) -> term.BNode | term.URIRef:
